...
//uses hybrid method of both KARATSUBA AND NORMAL MULTIPLICATION 
// SWITCHES DYNAMICALLY BASED ON THEIR SIZES

public class karatBig2Bench {
    ...
    public static BigInteger karatsuba(BigInteger x, BigInteger y) {
        // Threshold to switch to simple multiplication
        int THRESHOLD = 32;
        if (x.toString().length() <= THRESHOLD || y.toString().length() <= THRESHOLD) {
            return x.multiply(y);
        }
